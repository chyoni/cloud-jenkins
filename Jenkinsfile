        ...
        stage("Zip file transfer") {
            steps {
                script {
                    ...
                    sshCommand remote: remote, command: "cd /var/lib/jenkins/jobs/${JOB_NAME}/builds/16; zip -r report_included_css_file.zip cucumber-html-reports_*; curl -D- -u " + '$TBELL_JIRA_CWCHOI_USER'+':'+'$TBELL_JIRA_CWCHOI_PSW' + " -X POST -H 'X-Atlassian-Token: no-check' -F 'file=@report_included_css_file.zip' https://cwchoiit.atlassian.net/rest/api/3/issue/${JIRA_ISSUE_KEY}/attachments; rm -rf report_included_css_file.zip"
