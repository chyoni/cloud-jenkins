Feature: 통합뷰어 전체 기능별 실행 체크(초등 기준)
...
##### [독서] 메뉴 관련
  ...
  Scenario Outline: [독서 - AI맞춤] 구성 확인 (totalFunction_001)
    ...
    Examples:
      | contents |
      | 또래친구 책장 속 재미있는 콘텐츠-또래친구들이 좋아하는 #독 #동물 콘텐츠-나만 못 본 주간베스트-균형있게 읽어요-학교 공부가 쉬워져요 |
  ...
  ### 독서 - 타임챌린지 메뉴 사라짐 (Check 상태로 변경)
  #Scenario: [독서 - 타임챌린지] 구성 확인 (totalFunction_018)
  #  Given 독서 버튼 클릭
  #  When 독서 - "타임챌린지" 서브메뉴 클릭
  #  Then 타임챌린지 투데이 화면구성 확인

  #Scenario: [독서 - 타임챌린지] 기능 확인 (totalFunction_019)
  #  Given 독서 버튼 클릭
  #  When 독서 - "타임챌린지" 서브메뉴 클릭
  #  And 타임챌린지 콘텐츠 클릭
  #  Then 타임챌린지 콘텐츠 확인

  #Scenario: [독서 - 타임챌린지] 가이드 기능 확인 (totalFunction_020)
  #  Given 독서 버튼 클릭
  #  When 독서 - "타임챌린지" 서브메뉴 클릭
  #  And 타임챌린지 가이드 버튼 클릭
...
##### [라이브러리] 메뉴 관련 (실행 체크 부분 보완할지 검토/ 미리보기, 바로보기 등)
  ...
  # 멀티 터치북의 경우, 플레이어 닫기 기능이 정상 동작하지 않으므로 별도 시나리오로 분리
  @pass
  ...
  # 소리동요/소리동화 관련 콘텐츠 리스트가 보여지지 않음(추가 확인 필요)
  @pass
  Scenario Outline: [라이브러리 - 마이] 세트 메뉴별 기능 확인 (totalFunction_041)
    ...
    And 라이브러리 마이 세트 팝업에서 첫번째 콘텐츠 클릭
    And 5초 대기
    And Reading continue "No"
    Then 플레이어 실행 확인
    And 플레이어 종료하기
  ...
  #종이책 클릭 후 확인되지 않은 파일입니다. 메시지 뜨는 경우, return
  #콘텐츠 없는 경우, return
  ...
  Scenario: [라이브러리 - 전체메뉴] 구성 확인 (totalFunction_054)
    Given 라이브러리 버튼 클릭
...
##### [일반] 메뉴 관련 (독서앨범, 스마트올 백과 등)
  ...
  Scenario: [독서앨범 - 내 작품 보기] 기능 확인 (totalFunction_065)
    ...
    When 일반 뒤로가기 버튼 클릭
    And 독서앨범 내 작품 보기 "사용자 오디오이북" 탭 클릭
    And 독서앨범 내 작품 보기 탭 콘텐츠 클릭
    Then 독서앨범 내 작품 보기 탭 화면구성 확인
    When 일반 뒤로가기 버튼 클릭
    And 독서앨범 내 작품 보기 "일기" 탭 클릭
    And 독서앨범 내 작품 보기 탭 콘텐츠 클릭
    Then 독서앨범 내 작품 보기 탭 화면구성 확인
    When 일반 뒤로가기 버튼 클릭
    And 독서앨범 내 작품 보기 "사진" 탭 클릭
    And 독서앨범 내 작품 보기 탭 콘텐츠 클릭
    Then 독서앨범 내 작품 보기 탭 화면구성 확인
    When 일반 뒤로가기 버튼 클릭
  ...
  Scenario: [웅진 스마트올 - AI 학습센터] 방학 특강 기능 확인 (totalFunction_073)
    Given 웅진 스마트올 버튼 클릭
    When 웅진 스마트올 - "AI 학습센터" 서브메뉴 클릭
    And 웅진 스마트올 AI 학습센터 "방학 특강" 서브메뉴 클릭
    Then 웅진 스마트올 AI 학습센터 방학 특강 실행 확인

  Scenario: [웅진 스마트올 - AI 학습센터] 영어 기능 확인 (totalFunction_074)
    Given 웅진 스마트올 버튼 클릭
    When 웅진 스마트올 - "AI 학습센터" 서브메뉴 클릭
    When 웅진 스마트올 AI 학습센터 "영어" 서브메뉴 클릭
  ...
  Scenario: [웅진 스마트올 - AI 학습센터] 독서 기능 확인 (totalFunction_076)
    Given 웅진 스마트올 버튼 클릭
    When 웅진 스마트올 - "AI 학습센터" 서브메뉴 클릭
    When 웅진 스마트올 AI 학습센터 "독서" 서브메뉴 클릭
  ...
  Scenario: [학습 - 한글깨치기] 동영상 플레이어 실행 확인 (totalFunction_084)
    ...
    And 본학습단계 클릭
    And "15-1호 / 낱말 학습|자연 이름 익히기" 단계 클릭
    And 책이랑 글자랑 영역 콘텐츠 클릭
    Then 플레이어 실행 확인

  Scenario: [학습 - 유아수학] 동영상 플레이어 실행 확인 (totalFunction_085)
    Given 학습 버튼 클릭
    When "유아수학" 과목 클릭
    And "유아수학" 학습단계 확인 버튼 클릭
    And 유아수학 D단계 클릭
    And "5호 : 1. 9까지의 수(1)" 단계 클릭
    And 개념이 쏙쏙1 영역 콘텐츠 클릭
    Then 플레이어 실행 확인

  Scenario: [학습 - 수학] 동영상 플레이어 실행 확인 (totalFunction_086)
    Given 학습 버튼 클릭
    When "개정수학" 과목 클릭
  ...
  Scenario: [학습 - Vacabulary Master] 동영상 플레이어 실행 확인 (totalFunction_090)
    ...
    And "1호: 10 Sight Words―My Things" 단계 클릭
    And Chant 영역 콘텐츠 클릭
    And 5초 대기
    Then 플레이어 실행 확인
  ...
  Scenario: [학습 - 생각토론] 동영상 플레이어 실행 확인 (totalFunction_091)
    ...
    And 생각토론 C단계 클릭
    And "1호: 나와 가족 이야기 1(국어-너도나도)" 단계 클릭
  ...
  Scenario: [학습 - 북클럽중등] 동영상 플레이어 실행 확인 (totalFunction_095)
    Given 프로필 버튼 클릭
    When 북클럽 진도 학년 클릭
    And 관리자 비밀번호 입력
    And 교과 진도 중등 클릭
